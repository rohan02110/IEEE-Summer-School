...
DEFAULT_EVENT_DATES = "2026-09-24,2026-09-26,2026-09-27,2026-09-28,2026-09-29,2026-09-30,2026-10-01"
raw_dates = os.getenv("EVENT_DATES", DEFAULT_EVENT_DATES)
if not raw_dates or "2026-10-12" in raw_dates:
    raw_dates = DEFAULT_EVENT_DATES
EVENT_DATES = [d.strip() for d in raw_dates.split(",") if d.strip()]
...
VOLUNTEER_PIN = os.getenv("VOLUNTEER_PIN", "4821")
ADMIN_PIN = os.getenv("ADMIN_PIN", "917304")
...
async def api_mark(request: Request):
    ...
    try:
        ...
        msg = str(getattr(ex, "message", "") or ex)
        if "row-level security" in msg.lower() or "42501" in msg:
            return JSONResponse({"status": "error", "message": "Database RLS Error: SUPABASE_KEY in .env must be the service_role key."}, status_code=500)
        return JSONResponse({"status": "error", "message": f"Database Error: {msg}"}, status_code=500)
...
async function handle(rawCode){
  ...
  try{
    ...
    if(!r.ok){
      show('bad','✗','Database / Server Error',d.message||d.error||('HTTP error '+r.status));
      last='';
      return;
    }
    if(d.status==='marked'){
      ...
      show('bad','✗','Error',d.message||'Database error');
      last='';
    } else {
      show('bad','✗','Unknown code ('+code+')','Not registered in participant list');
    }
  }catch(err){
    show('bad','✗','Connection Failed','Please check server or internet connection');
